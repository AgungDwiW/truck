...
include_once "application/config/connectionASN.php";
...
class TableASN{
	...
	function join($tbl = '', $glue=[]){
		$this->__joins[] = ['tbl'=> $tbl, 'glue' => $glue];
	...
	function makeOutbound(){
		...
		// $base_path2 = dirname(__FILE__,3);
		global $conSL;
		// include  "{$base_path2}/application/config/connectionSL.php";
		$table = new Table($this->table_name);
		$table->mode = $this->mode;
		$table->condition = $this->condition;
		// $table->join = $this->join;
		foreach($this->__joins as $row){
			$table->join($row['tbl'], $row['glue']);
		}
		if ($this->mode == 'UPDATE'){
