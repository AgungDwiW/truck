...
class User extends ApiModel{
	...
	public static function checkLogin(){
		...
		if (!defined("PLANT_NAME"))
			define("PLANT_NAME",$_SESSION["plant_name"]);
		if (!defined("PLANTNAME"))
			define("PLANTNAME",$_SESSION["plant_name"]);
	}
    ...
    public static function login($username, $password){
		$response = User::GetAccessToken($username, $password, 420);
	...
	public static function GetAccessToken($username, $password)
    {
        ...
        try {
            // $call = new ApiClient();
            // $response = $call->ExecuteApi('POST', $url, $loginDto, null, false, false);
            // $responseDto = json_decode(preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $response[1]), true);
			$resp = self::post($loginDto, $url, false);

            $statusCode = $resp['status'];
			$responseDto = $resp['data'];
            if ($statusCode == 200) {
                if (!isset($_SESSION))
                    session_start();
