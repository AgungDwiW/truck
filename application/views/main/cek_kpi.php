...
//echo $day;
    ...
    FROM tb_ceklist
    WHERE hasil_pemeriksaan<>'Di Tolak di Pos 1' and month(tgbaca)='".$bln."' and plant_id='$plant_id'  ")or die(mysql_error());
    $A = mysqli_fetch_array($query_1);

    $query_2 = mysqli_query($con,"SELECT COUNT(hasil_pemeriksaan) as sisa
    FROM tb_ceklist
    WHERE hasil_pemeriksaan='Lanjut Pemeriksaan Gate 2' and month(tgbaca)='".$bln."' and plant_id='$plant_id'")or die(mysql_error());
    ...
    FROM tb_ceklist
    WHERE hasil_pemeriksaan<>'Di Tolak di Pos 1' and day(tgbaca)='".$day."' and plant_id='$plant_id'")or die(mysql_error());
    $C = mysqli_fetch_array($query_2);

    $query_4 = mysqli_query($con,"SELECT COUNT(hasil_pemeriksaan) as sisa
    FROM tb_ceklist
    WHERE hasil_pemeriksaan='Lanjut Pemeriksaan Gate 2' and day(tgbaca)='".$day."' and plant_id='$plant_id'")or die(mysql_error());
